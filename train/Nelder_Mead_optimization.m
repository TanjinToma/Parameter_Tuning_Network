function [params,optval,info,varargout] = Nelder_Mead_optimization(initialsimplex,lbs,ubs,reconfun,optfun,opt_spec)
...
% set the parameters in Nelder-Mead optimization (e.g., number of
% iterations, tolerance threshold, etc)
if ~exist('opt_spec','var') || isempty(opt_spec), opt_spec = struct(); end
if ~isfield(opt_spec,'niters') || isempty(opt_spec.niters), opt_spec.niters = 40; end
...
disp('evaluating initial simplex points to find the best initial params');
...
% save('coordmatrix.mat','coordmatrix');
% rank(coordmatrix)
volume = abs(det(coordmatrix))/factorial(Nparams);
%volume <= eps(prod(sqrt(sum(coordmatrix.^2,1)))) % previous checking criteria
if volume <= eps(mean(sqrt(sum(coordmatrix.^2,1))))
...
function volume = getvolume(simplex)
